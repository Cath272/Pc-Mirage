...
app.post("/produse_cos",function(req, res){
    ...
    if(req.body.ids_prod.length!=0){
        ...
        AccesBD.getInstanta().select({tabel:"componente", campuri:"nume,descriere,pret,capcitate_memorie,imagine".split(","),conditiiAnd:[`id in (${req.body.ids_prod})`]},
        function(err, rez){
            if(err)
                res.send([]);
            else
...
client.query("select id from componente", function(err, rez){
